...
int ppmCalc(int voltage){
	float ppm, resistance;

	resistance = (1023./(float)voltage)*5. - 1. * RLOAD;
...
int main(int argc, char *argv[]) {
    ...
    // 아틱 시간 설정
    setCurrentTime(2017, 9, 16, 22, 7, 0);
	date_showtime();
    ...
    while(1) {
		...
		if(sample.am_channel == 1){
			mqttDataLen = snprintf(mqttData, sizeof(mqttData), "{\"toxicGas\":%d}", sample.am_data);
